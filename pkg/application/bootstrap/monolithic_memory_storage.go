...
func (db *MonolithicMemoryDatabase) StoreEvent(ctx context.Context, event any) error {
	db.mu.Lock()
	defer db.mu.Unlock()
...
func (db *MonolithicMemoryDatabase) QueryEvents(ctx context.Context, filter any) ([]any, error) {
	db.mu.RLock()
	defer db.mu.RUnlock()

	results := make([]any, 0, len(db.events))
