# Phase 385 - Puller Runtime Summary Surface
...
Expose a read-only `puller` `/runtime/summary` route so operators can read
rollout posture, component posture, and metrics summary from one compact
runtime surface.
...
Add a focused `/runtime/summary` route for `puller` that returns:
...
- add `/runtime/summary` to the `puller` runtime mux
...
- `event-processor` parity work in the same phase

## Validation

- `GOROOT= GOCACHE=/tmp/chainpulse-go-build-cache go test ./cmd/microservices/puller/...`
- `scripts/dev-micro-loop.sh --mode fast --base HEAD`
- `./scripts/spec-approval-check.sh docs/specs/2026-04-02-architecture-phase385-puller-runtime-summary-surface.md`

## Exit Criteria

- `puller` exposes `/runtime/summary`.
