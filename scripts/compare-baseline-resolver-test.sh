...
current_file="${1:-build/migration-governance/baseline-resolver-test.prom}"
baseline_file="${2:-docs/operations/MIGRATION_BASELINE_RESOLVER_TEST_BASELINE.prom}"
output_dir="${3:-build/migration-governance}"
failure_mode="${CHAINPULSE_BASELINE_RESOLVER_TEST_DELTA_FAILURE_MODE:-warn}"

if [[ ! -f "$current_file" ]]; then
  echo "[baseline-resolver-test-delta] missing current resolver test metrics file: $current_file"
  exit 1
fi

if [[ ! -f "$baseline_file" ]]; then
  echo "[baseline-resolver-test-delta] baseline file not found, using current snapshot as baseline: $baseline_file"
...
case "$failure_mode" in
  warn|enforce)
    ;;
  *)
    echo "[baseline-resolver-test-delta] invalid CHAINPULSE_BASELINE_RESOLVER_TEST_DELTA_FAILURE_MODE: $failure_mode (expected warn|enforce)"
    exit 1
    ;;
esac

mkdir -p "$output_dir"
delta_tsv="$output_dir/baseline-resolver-test-delta.tsv"
delta_md="$output_dir/baseline-resolver-test-delta.md"
...
baseline_failed="$(metric_value "$baseline_file" "chainpulse_baseline_resolver_test_failed")"
current_failed="$(metric_value "$current_file" "chainpulse_baseline_resolver_test_failed")"
failed_delta=$((current_failed - baseline_failed))

baseline_status="$(metric_value "$baseline_file" "chainpulse_baseline_resolver_test_status")"
current_status="$(metric_value "$current_file" "chainpulse_baseline_resolver_test_status")"
...
{
  echo "# Baseline Resolver Test Delta"
  echo
...
echo "[baseline-resolver-test-delta] generated:"
echo "  - $delta_tsv"
echo "  - $delta_md"

if [[ "$failure_mode" == "enforce" && "$regression_status" != "none" ]]; then
  echo "[baseline-resolver-test-delta] regression detected: $regression_status"
  exit 1
fi
